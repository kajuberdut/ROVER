...
try:
    ...
    print("Error: PyYAML package is required for frontmatter parsing. Run: pip install pyyaml")
    sys.exit(1)
...
def extract_frontmatter(file_path: Path) -> dict:
    ...
    try:
        ...
        print(f"[ERROR] Invalid YAML syntax in {file_path.relative_to(VAULT_ROOT)}: {e}")
        return None
...
def validate_note(file_path: Path) -> list:
    ...
    if "id" in fm:
        ...
        if not re.match(r"^\d{12}$", val):
            errors.append(f"Invalid 'id': '{val}'. Must be 12-digit timestamp YYYYMMDDHHMM.")
    ...
    if "feature_status" in fm and fm["feature_status"] not in VALID_FEATURE_STATUSES:
        errors.append(f"Invalid 'feature_status': '{fm['feature_status']}'. Must be one of {VALID_FEATURE_STATUSES}.")
    ...
    if "stale_after" in fm and fm["stale_after"]:
        ...
        try:
            ...
            if stale_date < datetime.now():
                print(f"[WARNING] Note {rel_path} has expired stale_after date: {stale_val}")
        except ValueError:
            errors.append(f"Invalid 'stale_after' date format: '{stale_val}'. Expected YYYY-MM-DD.")
