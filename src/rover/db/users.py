...
from rover.db.schema import users, product_users
...
def upsert_user(sub: str, email: str | None, name: str | None) -> dict[str, Any]:
    with get_db_connection() as conn:
        ...
        if row:
            ...
            conn.execute(
                insert(users)
                .values(sub=sub, email=email, name=name, last_login=func.current_timestamp())
            )
        row = conn.execute(select(users).where(users.c.sub == sub)).fetchone()
...
def get_all_users() -> list[dict[str, Any]]:
    with get_db_connection() as conn:
        rows = conn.execute(select(users).order_by(users.c.role.asc(), users.c.name.asc())).fetchall()
...
def get_product_users(product_id: str) -> list[dict[str, Any]]:
    with get_db_connection() as conn:
        stmt = (
            ...
            .select_from(users.join(product_users, users.c.sub == product_users.c.user_sub))
...
def get_user_product_role(sub: str, product_id: str) -> str | None:
    with get_db_connection() as conn:
        row = conn.execute(
            select(product_users.c.role)
            .where(product_users.c.user_sub == sub, product_users.c.product_id == product_id)
        ).fetchone()
...
def set_product_user_role(sub: str, product_id: str, role: str) -> None:
    ...
    with get_db_connection() as conn:
        ...
        if row:
            ...
            conn.execute(
                insert(product_users)
                .values(user_sub=sub, product_id=product_id, role=role)
            )

def remove_product_user(sub: str, product_id: str) -> None:
    with get_db_connection() as conn:
        conn.execute(
            delete(product_users)
            .where(product_users.c.user_sub == sub, product_users.c.product_id == product_id)
        )
