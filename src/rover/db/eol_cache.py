...
def set_cached_eol_data(name: str, version: str, response_json: str) -> None:
    ...
    with get_db_connection() as conn:
        row = conn.execute(
            select(eol_cache.c.id).where(eol_cache.c.name == name, eol_cache.c.version == version)
        ).fetchone()
        if row:
            conn.execute(
                update(eol_cache)
            ...
            try:
                conn.execute(
                    insert(eol_cache).values(id=cache_id, name=name, version=version, response_json=response_json)
                )
