...
from rover.db.schema import repositories, images, major_components
...
def add_repository(url: str) -> str:
    ...
    try:
        ...
        with get_db_connection() as conn:
            row = conn.execute(select(repositories.c.id).where(repositories.c.url == url)).fetchone()
            return str(row[0])

def get_all_repositories() -> list[dict[str, Any]]:
    with get_db_connection() as conn:
        rows = conn.execute(select(repositories).order_by(repositories.c.created_at.desc())).fetchall()
        return [dict(row._mapping) for row in rows]

def get_repository(repo_id: str) -> dict[str, Any] | None:
    with get_db_connection() as conn:
        row = conn.execute(select(repositories).where(repositories.c.id == repo_id)).fetchone()
...
def add_image(name: str) -> str:
    ...
    try:
        ...
        with get_db_connection() as conn:
            row = conn.execute(select(images.c.id).where(images.c.name == name)).fetchone()
            return str(row[0])

def get_all_images() -> list[dict[str, Any]]:
    with get_db_connection() as conn:
        rows = conn.execute(select(images).order_by(images.c.created_at.desc())).fetchall()
        return [dict(row._mapping) for row in rows]
...
def add_major_component(name: str, version: str) -> str:
    ...
    try:
        with get_db_connection() as conn:
            conn.execute(insert(major_components).values(id=component_id, name=name, version=version))
            return component_id
    except IntegrityError:
        with get_db_connection() as conn:
            row = conn.execute(
                select(major_components.c.id)
                .where(major_components.c.name == name, major_components.c.version == version)
            ).fetchone()
            return str(row[0])

def get_all_major_components() -> list[dict[str, Any]]:
    with get_db_connection() as conn:
        rows = conn.execute(select(major_components).order_by(major_components.c.name.asc(), major_components.c.version.desc())).fetchall()
        return [dict(row._mapping) for row in rows]

def get_major_component(component_id: str) -> dict[str, Any] | None:
    with get_db_connection() as conn:
        row = conn.execute(select(major_components).where(major_components.c.id == component_id)).fetchone()
        return dict(row._mapping) if row else None
