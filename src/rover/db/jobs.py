...
def create_job(target_url: str, git_ref: str | None = None, target_type: str = "repo") -> str:
...
def get_all_jobs() -> list[dict[str, Any]]:
    with get_db_connection() as conn:
        rows = conn.execute(select(scan_jobs).order_by(scan_jobs.c.created_at.desc())).fetchall()
...
def get_semgrep_job(job_id: str) -> dict[str, Any] | None:
    with get_db_connection() as conn:
        row = conn.execute(select(semgrep_jobs).where(semgrep_jobs.c.id == job_id)).fetchone()
        return dict(row._mapping) if row else None

def get_semgrep_job_for_target(target_url: str, git_ref: str | None) -> dict[str, Any] | None:
    with get_db_connection() as conn:
...
def get_completed_semgrep_job_by_commit(commit_hash: str) -> dict[str, Any] | None:
    with get_db_connection() as conn:
        row = conn.execute(
            select(semgrep_jobs)
            .where(semgrep_jobs.c.resolved_commit == commit_hash, semgrep_jobs.c.status == 'completed')
...
def claim_next_job() -> dict[str, Any] | None:
    with get_db_connection() as conn:
        row = conn.execute(
            select(scan_jobs.c.id, scan_jobs.c.target_url, scan_jobs.c.git_ref, scan_jobs.c.target_type)
