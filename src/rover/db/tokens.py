...
def get_user_api_tokens(user_sub: str) -> list[dict[str, Any]]:
    with get_db_connection() as conn:
        rows = conn.execute(
            select(
                api_tokens.c.id,
                api_tokens.c.user_sub,
                api_tokens.c.name,
                api_tokens.c.permission,
    ...
    with get_db_connection() as conn:
        conn.execute(
            delete(api_tokens).where(
                api_tokens.c.id == token_id, api_tokens.c.user_sub == user_sub
            )
        )
...
def verify_api_token(token_string: str) -> dict[str, Any] | None:
    if not (
        ...
        or token_string.startswith("rover_r_")
        or token_string.startswith("rover_w_")
    ):
        return None
    ...
    with get_db_connection() as conn:
        row = conn.execute(
            select(api_tokens).where(api_tokens.c.token_hash == token_hash)
        ).fetchone()
        if row:
            conn.execute(
                update(api_tokens)
